#!/home/tanyag/anaconda3/bin/python
'''
...
parser = argparse.ArgumentParser(description='Extract segmented objects from field of view and save channels into separate images')
parser.add_argument('image_files', type=str, help='csv file with list of mips with seg files')
parser.add_argument('channels', type=str, help='json file with channel identifiers')
parser.add_argument('--out_dir', dest='out_dir', type=str, default=os.getcwd(), help='output directory where to save images')
...
def auto_contrast_fn(im_array):
    ...
    for i in range(hist[0].size): # iterate through all pixel values 0,1,2,...255
        if hist[0][i] >= limit: # if more pixels have this value than limit, set to zero
            pos = im_array_n == i
            im_array_n[pos] = 0 # assumes??? pix values with high counts are low value pixels eg 0,1,2,3
        elif limit > hist[0][i] > threshold:
            if low_thresh is None: # lowest pixel value with counts between limit & threshold
                low_thresh = i
            else:
                pass
        else:
    ...
    for i in range(hist[0].size):
        if limit < hist[0][hist[0].size-i-1] < threshold:
            ...
            high_thresh = hist[0].size-i-1 # highest pixel value between limit & threshold
            break
        else:
            pass
    print (low_thresh, high_thresh, np.min(im_array_n))
    out_array = exposure.rescale_intensity(im_array_n, in_range=(low_thresh, high_thresh)) # scale to 0,255
...
for filename in file_names:
    ...
    for cell in range(1, num_labels+1):
        ...
        label_crop = label_image[min(y): max(y), min(x): max(x)] # add one to max of slices?
        ...
        for c in channels.keys():
            ...
            imageio.imwrite(os.path.join(args.out_dir, basename +  '_cell' + str(cell) + '_C' + str(current_channel) + '.png'), cell_object_crop)
