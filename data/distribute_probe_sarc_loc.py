...
df = pd.read_csv(
    r"\\allen\aics\microscopy\Calysta\test\fish_struc_seg\sarc_classification_for_Rory.csv"
)
df = df.drop(["Unnamed: 0"], axis=1)
df = df.drop(["Unnamed: 0.1"], axis=1)

df = df.rename(
    columns={"fov_path": "original_fov_location", "cell_num": "napariCell_ObjectNumber"}
)
...
ds.distribute(
    ...
    message="Probes correlation with sarcomere organization",
)
